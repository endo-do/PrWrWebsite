...
export const Comparison = () => {
  ...
  return (
    <div className="space-y-10 text-white">
      <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-indigo-900/60 to-slate-950 p-8 shadow-2xl">
        <p className="text-xs uppercase tracking-[0.5em] text-white/60">Comparison lab</p>
        <h1 className="mt-4 font-display text-4xl">Which game is the harshest grind?</h1>
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-6">
        <h2 className="font-display text-2xl">Category swing since 2016</h2>
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              {games.map((game) => (
                <tr key={game.id}>
                  <td className="py-3 font-semibold">{game.icon} {game.name}</td>
                  {metricCategories.map((category) => {
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {comparisonInsights.map((insight) => {
          ...
          return (
            <article key={insight.gameId} className="rounded-3xl border border-white/5 bg-white/[0.04] p-5">
              <div className="flex items-center justify-between">
                <h3 className="font-display text-xl">{game?.icon} {game?.name}</h3>
                <span className={`text-sm font-semibold ${color}`}>
                  {insight.direction === 'harder' ? 'Harder' : 'Easier'}
                </span>
              </div>
              <p className="mt-2 text-lg font-semibold text-white">{insight.headline}</p>
