...
export const ComparisonPanel = () => (
  <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-xl">
    <header className="mb-6 flex flex-col gap-2">
      <p className="text-xs uppercase tracking-[0.3em] text-white/50">Who got harder?</p>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {comparisonInsights.map((insight) => {
        ...
        return (
          <article
            key={insight.gameId}
            className="rounded-2xl border border-white/5 bg-white/[0.04] p-4 transition hover:border-white/20"
          >
            ...
            <h4 className="mt-2 font-display text-xl text-white">{insight.headline}</h4>
